...
export function openTagEdit(g, evt) {
  ...
  // Measure the text bounding boxes in SVG coords (bypass group transform)
  const labelBBox = labelEl.getBBox();
  const valueBBox = valueEl.getBBox();
  const pad = 4; // small padding around the inputs
  ...
  // Translate bbox corners to screen coords via group CTM (picks up transforms)
  ...
  // Use absolute extents (in case of rotation we still want bounding rect)
  ...
  // Guarantee minimum width so short values still give room to type
  const finalW = Math.max(overlayW, 120);

  // Text alignment: center for top/bottom, left for left anchor, right for right anchor
  ...
  overlay.style.cssText = `
    ...
    z-index: 9999; box-sizing: border-box;
  `;
  // If the text anchor is left/right, nudge overlay so it visually lines up
  ...
  // Tab/Enter jumps between fields, then finishes
