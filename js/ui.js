...
import { addPlayer, rewrapTextBox, rewrapHeadline, updatePlayerArms, repositionTag, applyJerseyStyle, updateAllLinks, updateLadder, updatePoleColor, updateHoopColor, updateMannequinColor } from './elements.js';
...
function findRowForTarget(target) {
  for (const btn of document.querySelectorAll('.color-picker-btn')) {
    ...
    if (onclick.includes(`openColorPicker('${target}')`) || onclick.includes(`openColorPicker("${target}")`)) {
      return btn.closest('.picker-row');
    }
  }
  return null;
}
...
export function applyMannequinColor(swatchEl) {
  if (!S.selectedEl || S.selectedEl.dataset.type !== 'mannequin') return;
  updateMannequinColor(S.selectedEl, swatchEl.dataset.color);
  document.querySelectorAll('#mannequin-edit-section .color-swatch').forEach(sw => sw.classList.remove('active'));
  swatchEl.classList.add('active');
  trackElementEdited('mannequin', 'colour');
}
...
export function applyLadderRungs(v) {
  ...
  S.selectedEl.dataset.rungs = v;
  const out = document.getElementById('ladder-rungs-val');
  if (out) out.textContent = v;
  updateLadder(S.selectedEl);
  // the dashed select box is sized off the rung count — rebuild it
  const ring = S.selectedEl.querySelector('.select-ring');
  if (ring) {
    const hw = (parseInt(v, 10) * 8) / 2 + 4;
    ring.setAttribute('x', -hw); ring.setAttribute('width', hw * 2);
  }
  applyTransform(S.selectedEl);
  trackElementEdited('ladder', 'rungs');
}

export function applyLadderColor(swatchEl) {
  if (!S.selectedEl || S.selectedEl.dataset.type !== 'ladder') return;
  const color = swatchEl.dataset.color || '#FBBF24';
  S.selectedEl.dataset.ladderColor = color;
  updateLadder(S.selectedEl);
  document.querySelectorAll('#ladder-edit-section .color-swatch').forEach(sw => sw.classList.remove('active'));
  swatchEl.classList.add('active');
  trackElementEdited('ladder', 'colour');
}
...
export function applySize(val) {
  ...
  for (const el of targets) {
    ...
    if (t === 'player' || t === 'ball' || t === 'cone' || t === 'disc-cone' || t === 'small-goal' || t === 'ladder' || t === 'pole' || t === 'hoop' || t === 'mannequin' || t === 'vision' || t === 'marker' || t === 'spotlight' || t.startsWith('shadow') || t === 'tag') applyTransform(el);
...
export function applyRotation(val) {
  ...
  if (t.startsWith('shadow') || t === 'vision' || t === 'freeform' || t === 'pair' || t === 'small-goal' || t === 'ladder' || t === 'pole' || t === 'hoop' || t === 'mannequin') applyTransform(S.selectedEl);
  if (t === 'small-goal' || t === 'ladder' || t === 'pole' || t === 'hoop' || t === 'mannequin') updateHandlePositions(S.selectedEl);
